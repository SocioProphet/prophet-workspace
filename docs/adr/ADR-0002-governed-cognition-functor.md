...
| **GAP-4** | L1 (Θ, C, topology) | **Reconcile substrate doctrine ↔ code + run the falsification harness.** The Hopf-tower / dual-sector V⁺/V⁻ / M(Ψ) / S¹-order-cycle / monodromy constructs exist as `superconscious` doctrine + spectral-field math (`spectral_grounding.py`) but are **not named/tested in code** and the falsification harness has not run (ADR-0001 §9.7). **Doctrine + descriptor half reconciled** into one checkable reference — [`docs/architecture/world-model-substrate.md`](../architecture/world-model-substrate.md) + `schemas/world-model-substrate.schema.json` + `tools/validate_world_model_substrate.py` (names all 7 of W, binds each to a mechanism, teeth both ways); the code-naming + falsification-harness half remains open. | #86 |
