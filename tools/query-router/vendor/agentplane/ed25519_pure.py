"""Pure-Python Ed25519 (RFC 8032) — sign, verify, and public-key derivation.

Stdlib-only (hashlib). Adapted from the RFC 8032 reference implementation, which
is published for any purpose. Ed25519 signatures are deterministic, so signatures
produced here are byte-identical to any conformant implementation for the same
(seed, message) — the StopGateArtifact keeps zero third-party dependencies and
stays CI-portable, while signatures remain interoperable.

Keys are 32-byte seeds (private) / 32-byte compressed points (public); signatures
are 64 bytes.
"""
...
def _point_decompress(s: bytes):
    if len(s) != 32:
        raise ValueError("ed25519: invalid input length for decompression")
